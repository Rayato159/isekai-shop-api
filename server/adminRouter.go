...
func (s *echoServer) initAdminRouter(customMiddleware customMiddleware.CustomMiddleware) {
	...
	router.POST("", adminController.ItemCreating, customMiddleware.AdminAuthorizing)
	router.PATCH("/:itemID", adminController.ItemEditing, customMiddleware.AdminAuthorizing)
	router.DELETE("/:itemID", adminController.ItemArchiving, customMiddleware.AdminAuthorizing)
}
